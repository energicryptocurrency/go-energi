...
import (
	...
	"github.com/energicryptocurrency/go-energi/eth/filters"
	...
	"github.com/energicryptocurrency/go-energi/miner"
...
func New(ctx *node.ServiceContext, config *Config) (*Ethereum, error) {
	...
	if !config.SkipBcVersionCheck {
		...
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, Energi Core %s only supports v%d", *bcVersion, params.VersionWithMeta, core.BlockChainVersion)
...
func CreateConsensusEngine(ctx *node.ServiceContext, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db ethdb.Database) consensus.Engine {
	...
	if chainConfig.Energi != nil {
		return energi.New(chainConfig.Energi, db)
	}
