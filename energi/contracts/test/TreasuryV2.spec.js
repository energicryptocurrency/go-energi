...
contract("TreasuryV2", async accounts => {
    const s = {
        artifacts,
        accounts,
        assert,
    ...
    after(async () => {
        const impl = await TreasuryV2.new(s.proxy.address, s.registry.address, common.superblock_cycles);
    ...
    describe('ITreasury', () => {
        ...
        const fee_amount = toBN(toWei('302', 'ether'));
        ...
        it ('should collect rejected proposals', async () => {
            ...
            await s.reward_abi.reward();
            expect(toBN(await s.treasury_abi.balance()).sub(orig_bal).toString())
                .equal(toBN(toWei('968.2', 'ether')).toString());

            // another 2
            await common.moveTime(web3, (4*def_period/100)+1);
            await s.reward_abi.reward();
            expect(toBN(await s.treasury_abi.balance()).sub(orig_bal).toString())
                .equal(toBN(toWei('1572.2', 'ether')).toString());

            // remaining 95
            await common.moveTime(web3, (96*def_period/100)+1);
            await s.reward_abi.reward();
            expect(toBN(await s.treasury_abi.balance()).sub(orig_bal).toString())
                .equal(toBN(toWei('30564.2', 'ether')).toString());
        });
        ...
        it ('should correctly distribute full payouts', async() => {
            ...
            await s.reward_abi.reward();
            expect(toBN(await s.treasury_abi.balance()).sub(orig_bal).toString())
                .equal(toBN(toWei('0.600', 'ether')).toString());

            await common.moveTime(web3, def_period+1);
            await s.reward_abi.reward();
            expect(toBN(await s.treasury_abi.balance()).sub(orig_bal).toString())
                .equal(toBN(toWei('301.2', 'ether')).toString());
        });
        ...
        it ('should correctly distribute partial payouts', async() => {
            ...
            await s.treasury_abi.propose(
                ...
                { value: orig_bal.mul(mul1).mul(toBN(2)).div(toBN(1000)).add(toBN(300000000000000000000)), from: owner3 }
            );
            const proposal1 = await get_proposal();
            await s.treasury_abi.propose(
                orig_bal.mul(mul2), '302', def_period,
                { value: orig_bal.mul(mul2).mul(toBN(2)).div(toBN(1000)).add(toBN(300000000000000000000)), from: owner4 }
            );
            const proposal2 = await get_proposal();
