...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	if err = checkPort(client.server, nodePort); err != nil {
		log.Warn("Wallet devp2p port seems unreachable: server", client.server,
			"port", nodePort, "err", err)
	}
	rpcPort := infos.portmap["39796/tcp"]
	if err = checkPort(client.server, rpcPort); err != nil {
		log.Warn("Wallet RPC port seems unreachable: server", client.server,
			"port", rpcPort, "err", err)
	}
